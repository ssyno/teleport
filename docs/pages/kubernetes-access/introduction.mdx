...
Teleport Kubernetes Service instance on the cluster you want to register.  We'll
show you how to do this in our [Getting Started Guide](getting-started.mdx).

A single instance of the Teleport Kubernetes Service can protect multiple
Kubernetes clusters. This is particularly useful when the Kubernetes clusters'
nodes can be downsized to zero, but you still want to be able to access the
Kubernetes API. You can register multiple clusters manually, or set up the
Teleport Discovery Services to detect Kubernetes clusters in your cloud account. 
...
access controls to manage the way users access your cluster, including
Kubernetes resources like pods, secrets, and configmaps that a user is allowed
to manage.
